...
s2_we = T{:,6}-offset_voltage; %Column 6 reading and extracting the offset value
s2_ae = T{:,7}-offset_voltage; %Column 7 reading and extracting the offset value

s2_c=(s2_we - s2_ae); % Corrected final values of the Sensor 2.
...
s3_we = T{:,8}-offset_voltage; %Column 8 reading and extracting the offset value
s3_ae = T{:,9}-offset_voltage; %Column 9 reading and extracting the offset value

s3_c=(s3_we - s3_ae); % Corrected final values of the Sensor 3.

s3_len = length(s3_c); % Length of the data
fprintf('\n');
fprintf('Length of the Sensor 3 data reading =');
disp(s3_len);

s4_we = T{:,10}-offset_voltage; %Column 10 reading and extracting the offset value
s4_ae = T{:,11}-offset_voltage; %Column 11 reading and extracting the offset value

s4_c=(s4_we - s4_ae); % Corrected final values of the Sensor 4.
...
zoom_in_data_table = renamevars(zoom_in_data_table,["Var1"],["Nums1"]);
